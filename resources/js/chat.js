...
class ChatManager {
    ...
    createMessageHtml(message) {
        ...
        // Detect attachment and type
        const hasAttachment = !!message.attachment_path;
        const ext = hasAttachment ? (message.attachment_path.split('.').pop() || '').toLowerCase() : '';
        const isImage = hasAttachment && ['jpg','jpeg','png','gif','webp','bmp'].includes(ext);
        const attachmentUrl = hasAttachment ? `/storage/${message.attachment_path}` : null;
        ...
        if (hasAttachment) {
            if (isImage) {
                attachmentHtml = `
                    <div class="mb-2">
                        <img src="${attachmentUrl}" alt="Attachment" class="max-w-full h-auto rounded-md" />
                    </div>
                `;
            } else {
                const fileName = this.escapeHtml(message.attachment_path.split('/').pop());
                const linkBg = isFromCurrentUser ? 'bg-white/10 text-white' : 'bg-black/5 text-gray-800';
                attachmentHtml = `
                    <div class="mb-2">
                        <a href="${attachmentUrl}" target="_blank" class="flex items-center space-x-2 p-2 ${linkBg} rounded-md">
