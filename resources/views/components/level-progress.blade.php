        ...
        @if($amountNeeded !== null)
            <div class="space-y-2">
                ...
                <p class="text-xs text-neutral-500">
                    Butuh <span class="text-[#fe2c55] font-medium">Rp{{ number_format($amountNeeded, 0, ',', '.') }}</span> lagi untuk 
                    @if($currentLevel + 1 == 6)
                        mencapai status {{ $levelRequirements[6]['badge'] }}
                    @else
                        naik ke {{ $levelRequirements[$currentLevel + 1]['badge'] }}
                    @endif
                </p>
            </div>
        @else
