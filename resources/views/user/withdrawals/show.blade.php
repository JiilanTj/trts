                            ...
                            <div class="flex items-center gap-4 mb-4">
                                ...
                                <div>
                                    <div class="flex items-center gap-2 mb-2">
                                        <h3 class="text-lg font-semibold text-white">{{ $withdrawal->getStatusLabel() }}</h3>
                        ...
                        <div class="p-4">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <div class="text-white font-medium">{{ $withdrawal->account_name }}</div>
                                </div>
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="mb-8">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-white mb-2">Detail Penarikan</h1>
                <p class="text-neutral-400">Informasi lengkap tentang permintaan penarikan saldo</p>
            </div>
            <div class="flex items-center gap-3">
                @if($withdrawal->canBeCancelled())
                <form action="{{ route('user.withdrawals.cancel', $withdrawal) }}" method="POST" class="inline-block">
                    ...
                    <button type="submit" 
                            onclick="return confirm('Yakin ingin membatalkan penarikan ini? Saldo akan dikembalikan ke akun Anda.')"
                            class="inline-flex items-center gap-2 px-4 py-2 bg-red-600 hover:bg-red-500 text-white rounded-lg font-medium transition-colors">
    ...
    @if(session('success'))
    <div class="mb-6 bg-green-900/50 border border-green-500/50 rounded-lg p-4">
        <div class="flex items-center gap-3">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-neutral-800/30 border border-neutral-700 rounded-xl overflow-hidden">
                <div class="px-6 py-4 border-b border-neutral-700">
                    <h2 class="text-lg font-semibold text-white">Status Penarikan</h2>
                </div>
                ...
                <div class="p-6">
                    <div class="flex items-center gap-4 mb-6">
                        ...
                        <div>
                            <div class="flex items-center gap-2 mb-2">
                                <h3 class="text-xl font-semibold text-white">{{ $withdrawal->getStatusLabel() }}</h3>
                                <span class="px-2 py-1 text-xs font-medium rounded-full
                                    @if($withdrawal->status === 'pending') bg-yellow-500/20 text-yellow-300
                                    @elseif($withdrawal->status === 'processing') bg-blue-500/20 text-blue-300
                                    @elseif($withdrawal->status === 'completed') bg-green-500/20 text-green-300
                                    @elseif($withdrawal->status === 'rejected') bg-red-500/20 text-red-300
                                    @else bg-neutral-500/20 text-neutral-300
                                    @endif">
                                    {{ $withdrawal->status }}
                                </span>
                            </div>
                    ...
                    @if($withdrawal->notes)
                    <div class="bg-neutral-700/50 border border-neutral-600 rounded-lg p-4">
                        <h4 class="text-white font-medium mb-2">Catatan:</h4>
                        <p class="text-neutral-300 text-sm">{{ $withdrawal->notes }}</p>
                    </div>
                    @endif
                </div>
            </div>
            ...
            <div class="bg-neutral-800/30 border border-neutral-700 rounded-xl overflow-hidden">
                <div class="px-6 py-4 border-b border-neutral-700">
                    <h2 class="text-lg font-semibold text-white">Informasi Rekening</h2>
                </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div class="text-white font-medium text-lg">{{ $withdrawal->bank_name }}</div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="p-6">
                    <div class="space-y-4">
                        ...
                        <div class="flex gap-4">
                            ...
                            <div class="flex-1 min-w-0">
                                <h4 class="text-white font-medium">Pengajuan Dibuat</h4>
                                <p class="text-neutral-400 text-sm">{{ $withdrawal->created_at->format('d M Y, H:i:s') }}</p>
                                <p class="text-neutral-500 text-xs">{{ $withdrawal->created_at->diffForHumans() }}</p>
                            </div>
                        </div>
                        ...
                        <div class="flex gap-4">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-neutral-400 text-sm">{{ $withdrawal->updated_at->format('d M Y, H:i:s') }}</p>
                                <p class="text-neutral-500 text-xs">{{ $withdrawal->updated_at->diffForHumans() }}</p>
                                @if($withdrawal->processedBy)
                                <p class="text-neutral-500 text-xs mt-1">oleh {{ $withdrawal->processedBy->name }}</p>
                                @endif
                            </div>
                        </div>
                        @endif
        ...
        <div class="space-y-6">
            ...
            <div class="bg-neutral-800/30 border border-neutral-700 rounded-xl overflow-hidden">
                <div class="px-6 py-4 border-b border-neutral-700">
                    <h2 class="text-lg font-semibold text-white">Rincian Jumlah</h2>
                </div>
                
                <div class="p-6 space-y-4">
                    <div class="flex justify-between items-center">
                        <span class="text-neutral-400">Jumlah Penarikan</span>
                        <span class="text-white font-semibold">Rp {{ number_format($withdrawal->amount, 0, ',', '.') }}</span>
                    </div>
                    
                    @if($withdrawal->admin_fee > 0)
                    <div class="flex justify-between items-center">
                        <span class="text-neutral-400">Biaya Admin</span>
                        <span class="text-yellow-400 font-semibold">Rp {{ number_format($withdrawal->admin_fee, 0, ',', '.') }}</span>
                    </div>
                    ...
                    <div class="flex justify-between items-center">
                        <span class="text-white font-medium">Total Dipotong</span>
                        <span class="text-red-400 font-bold text-lg">Rp {{ number_format($withdrawal->total_deducted, 0, ',', '.') }}</span>
                    </div>
                    @endif
                </div>
            </div>
            ...
            <!-- Contact Support -->
            <div class="bg-gradient-to-br from-blue-600/20 to-purple-600/20 border border-blue-500/30 rounded-xl p-6">
                <div class="flex items-center gap-3 mb-4">
                    ...
                    <h3 class="text-white font-semibold">Butuh Bantuan?</h3>
                </div>
                <p class="text-blue-200/80 text-sm mb-4">
                    Jika ada pertanyaan tentang penarikan ini, hubungi tim support kami.
                </p>
                <a href="{{ route('user.support.tickets.create') }}" 
                   class="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-lg text-sm font-medium transition-colors w-full justify-center">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                    </svg>
                    Hubungi Support
            ...
            <div class="bg-neutral-800/30 border border-neutral-700 rounded-xl overflow-hidden">
                ...
                <div class="p-6 space-y-3">
                    <a href="{{ route('user.withdrawals.create') }}" 
                       class="flex items-center gap-3 p-3 bg-neutral-700/50 hover:bg-neutral-700 border border-neutral-600 rounded-lg transition-colors group">
                        <div class="w-8 h-8 rounded-lg bg-amber-500/20 flex items-center justify-center group-hover:bg-amber-500/30">
                            <svg class="w-4 h-4 text-amber-400" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4" />
                            </svg>
                        ...
                        <div>
                            <h4 class="text-white font-medium text-sm">Penarikan Baru</h4>
                            <p class="text-neutral-400 text-xs">Ajukan penarikan lainnya</p>
                        </div>
                    </a>
                    ...
                    <a href="{{ route('user.withdrawals.index') }}" 
                       class="flex items-center gap-3 p-3 bg-neutral-700/50 hover:bg-neutral-700 border border-neutral-600 rounded-lg transition-colors group">
                        ...
                        <div>
                            <h4 class="text-white font-medium text-sm">Daftar Penarikan</h4>
                            <p class="text-neutral-400 text-xs">Lihat semua penarikan</p>
                        </div>
                    </a>
                </div>
            </div>
