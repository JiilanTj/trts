                            ...
                            <div>
                                ...
                                <div class="flex items-center space-x-6 mt-2">
                                    <div class="text-left">
                                        ...
                                        <p class="text-[10px] uppercase tracking-wide text-neutral-400 mt-0.5">{{ $notifications->count() }} Pesan Sistem</p>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            @if($notifications->count() > 0)
                    ...
                    @foreach($notifications as $notification)
                        ...
                        <div class="flex items-start gap-3 px-3 py-2 rounded-lg border transition
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                @if($isUnread)
                                    <button onclick="markAsRead({{ $notification['id'] }})" class="mt-1 text-[11px] text-[#FE2C55] hover:underline">Tandai sudah dibaca</button>
                                @endif
                            </div>
                        </div>
                    @endforeach
                ...
                <div class="mt-5 flex items-center justify-between text-xs px-3 py-2 rounded-md bg-[#23272b] border border-neutral-700/40">
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-neutral-300 font-medium">{{ $notifications->count() }} notifikasi ({{ $unreadCount }} belum dibaca)</span>
                    </div>
                    @if($unreadCount > 0)
                        <button onclick="markAllAsRead()" class="text-[#25F4EE] hover:underline">Tandai semua</button>
                    @endif
                </div>
            @else
                <!-- Empty State -->
                <div class="text-center py-12">
                    <div class="w-16 h-16 mx-auto mb-4 rounded-full bg-neutral-800/50 flex items-center justify-center">
                        <svg class="w-8 h-8 text-neutral-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-5 5v-5zM3 3v18l5-5h14V3H3z"/>
                        </svg>
                    </div>
                    <h3 class="text-lg font-medium text-neutral-300 mb-2">Belum Ada Notifikasi</h3>
                    <p class="text-sm text-neutral-500 mb-6">Notifikasi sistem akan muncul di sini ketika ada aktivitas baru</p>
                    <a href="{{ route('dashboard') }}" class="inline-flex items-center px-4 py-2 rounded-lg bg-[#FE2C55] text-white text-sm font-medium hover:bg-[#FE2C55]/90 transition">
