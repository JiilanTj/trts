        ...
        <div class="px-4 sm:px-6 lg:px-8 py-6">
            ...
            <div class="space-y-3">
                @foreach($notifications as $notification)
                    <div class="rounded-xl border border-[#2c3136] bg-[#23272b] shadow-sm relative overflow-hidden {{ !$notification['read'] ? 'ring-1 ring-[#FE2C55]/20' : '' }}">
                        <!-- Unread indicator -->
                            ...
                            <div class="flex items-start space-x-3">
                                ...
                                <div class="flex-shrink-0">
                                    @php
                                        ...
                                        $colorClass = $iconColors[$notification['color']] ?? 'from-gray-500/20 to-gray-600/20 text-gray-400';
                                    @endphp
                                    ...
                                    <div class="w-10 h-10 rounded-full bg-gradient-to-br {{ $colorClass }} flex items-center justify-center">
                                        @switch($notification['icon'])
                                            ...
                                            @case('cog')
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.757.426 1.757 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.757-2.924 1.757-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.757-.426-1.757-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.607 2.273.07 2.573-1.065z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                                </svg>
                                                @break
                                            @case('gift')
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            ...
                                            @default
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-5 5v-5zM3 3v18l5-5h14V3H3z"></path>
                                                </svg>
                                        @endswitch
                                    </div>
                                </div>
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center justify-between">
                                        <h3 class="text-sm font-semibold text-neutral-100">{{ $notification['title'] }}</h3>
                                        @if(!$notification['read'])
                                            <div class="w-2 h-2 bg-[#FE2C55] rounded-full flex-shrink-0"></div>
                                        @endif
                                    </div>
                                    <p class="text-sm text-neutral-300 mt-1 leading-relaxed">{{ $notification['message'] }}</p>
                                    <div class="flex items-center justify-between mt-3">
                                        <span class="text-xs text-neutral-400">{{ $notification['time'] }}</span>
                                        @if(!$notification['read'])
                                            <button class="text-xs text-[#FE2C55] hover:text-[#FE2C55]/80 font-medium">
                                                Tandai sudah dibaca
                                            </button>
                                        @endif
                                    </div>
            ...
            <!-- Summary Card -->
            <div class="mt-6 rounded-xl border border-[#2c3136] bg-[#23272b]/50 p-4">
                <div class="flex items-center justify-between">
                    <div class="flex items-center space-x-3">
                        <div class="w-8 h-8 rounded-full bg-gradient-to-br from-[#FE2C55]/20 to-[#25F4EE]/20 flex items-center justify-center">
                            <svg class="w-4 h-4 text-neutral-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-5a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                            </svg>
                        </div>
                        <div>
                            <p class="text-sm font-medium text-neutral-200">Total Notifikasi</p>
                            <p class="text-xs text-neutral-400">{{ count($notifications) }} pesan, {{ $unreadCount }} belum dibaca</p>
                        </div>
                    </div>
                    <div class="text-right">
                        <button class="text-xs text-[#25F4EE] hover:text-[#25F4EE]/80 font-medium">
